$(function () {
  ...
  socket.on("dataroomessage", (data) => {
    for (let i = data.dataroomMessage.length - 1; i >= 0; i--) {
      if (data.dataroomMessage[i].username === userName) {
        $("#content").append(`<div class="receiver">
                                <div>
                                        <div>${data.dataroomMessage[i].chattime}</div>
                                    <strong style="font-size: 1.5em;">
                                    ${data.dataroomMessage[i].username} &nbsp;                                    
                                    </strong>
                                </div>
                                <div>
                                    <div class="right_triangle"></div>
                                    <span>&nbsp;&nbsp; ${data.dataroomMessage[i].message}</span>
                                </div>
                            </div>`);
      } else {
        $("#content").append(`<div class="sender">
