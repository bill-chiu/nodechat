  ...
  socket.on("createRoom", (data) => {
    ...
    var query1 = con.query(
      ...
      function (error, results, fields) {
        ...
        //如果不存在 新增房間
        if (roomRows == 0) {
          var query = con.query("INSERT INTO chatroom SET ?", post, function (
            error,
            results,
            fields
          ) {
  ...
  socket.on("sendMessage", (data) => {
    for (let _user of users) {
      if (_user.username === data.username) {
        ...
        let time = moment(new Date()).tz("Asia/Taipei").format("YYYY/MM/DD HH:mm");
